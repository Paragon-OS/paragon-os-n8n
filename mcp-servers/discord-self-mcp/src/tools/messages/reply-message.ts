...
import type { ReplyMessageArgs } from '../../types/tool-args.js'
...
export async function replyMessage(
  ...
  const { channelId, messageId, content, filePaths, maxFileSizeMB } = args
  ...
  try {
    ...
    const response: any = {
      ...
      content: content,
      timestamp: replyMessage.createdTimestamp,
      replyTo: messageId,
    ...
    throw new McpError(
      ...
      `Failed to reply to message: ${errorMsg}`,
    )
  }
}

export const replyMessageTool: ToolDefinition = {
  name: 'discord_reply_message',
  description: 'Reply to a specific Discord message. Creates a message that references the original message. Accepts channel ID or user ID. Supports file attachments.',
  inputSchema: {
    type: 'object',
    properties: {
      channelId: {
